...
const { EMAIL_SMTP_SERVER, PORT_SMTP_SERVER, EMAIL_USER, EMAIL_PASSWORD, WEBSITE_HOST } = process.env;
    ...
    const mailOptions = {
        ...
        html: `<div style="text-align: center;">
            ...
            <p style="text-align: center;">
                Cliquez sur ce <a href="${WEBSITE_HOST}/users/verify/${uniqueString}" style="text-decoration:none"><strong>lien</strong></a> pour valider votre email !
            </p>
            <br>
            <p style="overflow-wrap: break-word">
                Si le lien ne fonctionne pas, veuillez recopier ce lien dans votre navigateur :
                <br>
                ${WEBSITE_HOST}/users/verify/${uniqueString}
            </p>
        `
    };
...
usersController.verifyEmail = async (req, res) => {
    ...
    if (!user) {
        ...
        user.uniqueString = "";
        await user.save();
	req.flash('success_msg', `Votre adresse email ${user.email} a bien été validée !`);
...
usersController.renderSignInForm = async (req, res) => {
    ...
    if (user) {
        ...
        res.render('users/signin', {referer: req.headers.referer.slice(19)});
    }
};
...
const sendEmailCheckEmail = (email, uniqueString, imgLogo) => {
    ...
    const mailOptions = {
        ...
        <hr>
        <p style="text-align: center;">
            Cliquez sur ce <a href="${WEBSITE_HOST}/users/check-email/${uniqueString}" style="text-decoration:none"><strong>lien</strong></a> réinitialiser votre mot de passe !
        </p>
        <br>
        <p style="overflow-wrap: break-word">
            Si le lien ne fonctionne pas, veuillez recopier ce lien dans votre navigateur :
            <br>
            ${WEBSITE_HOST}/users/check-email/${uniqueString}
        </p>
        `
    };
